...
class NodeManager
{
    ...
    /**
     * @var \Monolith\CMSBundle\Repository\NodeRepository
     */
    ...
    public function buildModulesData(Request $request, array $nodes)
    {
        ...
        foreach ($prioritySorted as $nodes) {
            ...
            foreach ($nodes as $node) {
                ...
                // Выполняется модуль, все параметры ноды берутся в \Monolith\CMSBundle\Listener\ModuleControllerModifierListener
                ...
                foreach ($reflector->getMethods() as $method) {
                    if ($method->isPublic()
                        ...
                        foreach ($method->getParameters() as $parameter) {
                            if (!empty($parameter->getType()) and $parameter->getType()->getName() == 'Monolith\CMSBundle\Entity\Node') {
